function [mask, bestpaths, seam, new_copy_count] = cut_enlarge_k_seam(errpatch, copy_count)
% mask = cut(errpatch)
%
...
seam(bestpaths(end), end) = 1;
new_copy_count(bestpaths(end), end) = new_copy_count(bestpaths(end), end)+1;
...
for x = numel(bestpaths):-1:2
    ...
    new_copy_count(bestpaths(x-1), x-1) = copy_count(bestpaths(x-1), x-1) + 1;
end
